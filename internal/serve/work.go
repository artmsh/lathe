...
//
// Claiming a job dequeues it, so this GET mutates state and carries the same
// sameOrigin guard as the POST endpoints. The worker is the CLI, which sends no
// Origin header, so the guard admits it (local or remote) while rejecting a
// cross-site page that would otherwise steal jobs from the real worker over a
// widened --bind. Genuinely read-only GETs stay open so tutorials remain
// readable across the LAN.
