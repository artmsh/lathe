...
<script>
  ...
  (function(){
    ...
    var excerpt = '';
    // The server caps the excerpt at 4096 *bytes*; JS string length is
    // characters, and rendered prose carries typographic quotes and dashes that
    // cost 3 bytes each. Truncate on the encoded length so a large selection is
    // trimmed here rather than bounced back as a 400.
    var MAX_EXCERPT_BYTES = 4096;
    function capBytes(text, max){
      if (typeof TextEncoder === 'undefined') return text.slice(0, Math.floor(max / 3));
      var enc = new TextEncoder();
      if (enc.encode(text).length <= max) return text;
      var out = text;
      while (out.length && enc.encode(out).length > max) out = out.slice(0, Math.floor(out.length * 0.9) || 0);
      return out;
    }
    ...
    // A scroll, or the selection collapsing on its own, is accidental — mobile
    // text selection routinely fires a scroll — so it never discards a running
    // job or a report the reader hasn't read yet. A click outside and Escape
    // are deliberate and hide a resolved popup; see the listeners below.
    function hideIfIdle(){
    ...
    function onSelect(){
      ...
      // A fresh selection supersedes whatever the last correction reported.
      if (resolved) hide();
      excerpt = capBytes(text, MAX_EXCERPT_BYTES);
    ...
    // A click or tap outside is deliberate, so it dismisses a shown result too —
    // otherwise the handoff block, which never resolves on its own, would trap
    // the popup over the article on a device with no Escape key.
    document.addEventListener('mousedown', function(ev){
      if (!popup.hidden && !popup.contains(ev.target) && !pending) hide();
    });
